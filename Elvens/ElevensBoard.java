...
public class ElevensBoard extends Board {
    ...
    public boolean isLegal(List<Integer> selectedCards) {
        ...
        if (selectedCards.size() == 2 && findPairSum11(selectedCards).size() == 2)  {
            return true;
        }
        
        if (selectedCards.size() == 3 && findJQK(selectedCards).size() == 3) {
            return true;
        } 
        ...
        for (int sk1 = 0; sk1 < selectedCards.size(); sk1++) {
            ...
            for (int sk2 = sk1 + 1; sk2 < selectedCards.size(); sk2++) {
                ...
                if (cardAt(k1).pointValue() + cardAt(k2).pointValue() == 11) {
                    cardsToReturn.add(new Integer(k1));
                    cardsToReturn.add(new Integer(k2));
    ...
    private List<Integer> findJQK(List<Integer> selectedCards) {
        ...
        {
            cardsToReturn.add(new Integer(foundJack));
            cardsToReturn.add(new Integer(foundQueen));
            cardsToReturn.add(new Integer(foundKing));
        }
        return cardsToReturn;
    }
